...
torch.set_printoptions(precision=3, edgeitems=6,
                       sci_mode=False, threshold=3000)
...
parser.add_argument('-plm_model_class', type=str,
                    default='bert', help='pre-trained language model class')
parser.add_argument('-plm_model_name', type=str,
                    default='bert-base-uncased', help='pre-trained language model name')
parser.add_argument('-hidden_size', type=int, default=256,
                    help="hidden size of lstm")
parser.add_argument('-lr', type=float, default=3e-5, help="learning rate")
parser.add_argument('-dropout', type=float, default=0.4, help="dropout rate")
parser.add_argument('-loc_loss', type=float, default=0.3,
                    help="hyper-parameter to weight location loss")
parser.add_argument('-attn_loss', type=float, default=0.5,
                    help="hyper-parameter to weight attention loss")
parser.add_argument('-max_grad_norm', default=1.0,
                    type=float, help="Max gradient norm")
parser.add_argument('-grad_accum_step', default=1, type=int,
                    help='gradient accumulation steps')

# training parameters
parser.add_argument(
    '-mode', type=str, choices=['train', 'test'], default='train', help="train or test")
...
parser.add_argument('-epoch', type=int, default=100,
                    help="number of epochs, use -1 to rely on early stopping only")
...
parser.add_argument('-report', type=int, default=2,
                    help="report frequence per epoch, should be at least 1")
parser.add_argument('-train_set', type=str,
                    default="data/train.json", help="path to training set")
parser.add_argument('-dev_set', type=str,
                    default="data/dev.json", help="path to dev set")
parser.add_argument('-no_cuda', action='store_true',
                    default=False, help="if true, will only use cpu")

# test parameters
parser.add_argument('-test_set', type=str,
                    default="data/test.json", help="path to test set")
parser.add_argument('-restore', type=str, default=None,
                    help="path to saved checkpoint")
parser.add_argument('-dummy_test', type=str, default="data/dummy-predictions.tsv",
                    help="path to prediction file template")
parser.add_argument('-output', type=str, default=None,
                    help="path to store prediction outputs")
...
parser.add_argument('-finetune', action='store_true',
                    default=False, help='if true, fine-tune the bert encoder')
parser.add_argument('-no_wiki', action='store_true', default=False,
                    help='if true, use the vanilla PLM from huggingface')
...
def save_model(ckpt_dir, model_name, model: nn.Module, optimizer):
    ...
    if not opt.ckpt_dir:
        print(
            "[ERROR] Intended to store checkpoint but no checkpoint directory is specified.")
...
def train():
    ...
    if opt.ckpt_dir:
        if opt.ckpt_dir.endswith('/'):
            tb_writer = SummaryWriter(logdir=os.path.join(
                'runs', opt.ckpt_dir.split('/')[-2]))
        else:
            tb_writer = SummaryWriter(logdir=os.path.join(
                'runs', opt.ckpt_dir.split('/')[-1]))
    else:
        tb_writer = SummaryWriter()

    train_set = ProparaDataset(
        opt.train_set, opt=opt, tokenizer=plm_tokenizer, is_test=False)

    train_batch = DataLoader(
        dataset=train_set, batch_size=opt.batch_size, shuffle=True, collate_fn=Collate())
    dev_set = ProparaDataset(opt.dev_set, opt=opt,
    ...
    if opt.restore is not None:
        ...
        optim_state_dict = torch.load(
            os.path.join(opt.ckpt_dir, "optimizer.pt"))
        optimizer.load_state_dict(optim_state_dict)
        print(
            f'[INFO] Loaded model and optimizer from {opt.ckpt_dir}, resume training...')
    ...
    while epoch_i < opt.epoch:
        ...
        for batch in train_batch:
            ...
            num_cands = torch.IntTensor(
                [meta['total_loc_cands'] + 1 for meta in metadata])  # +1 for unk
            ...
            if batch_cnt % opt.grad_accum_step == 0:
                ...
                if batch_cnt in report_batch:
                    ...
                    total_accuracy = (
                        report_state_correct + report_loc_correct) / (report_state_pred + report_loc_pred)
                    ...
                    tb_writer.add_scalar(
                        'train_state_loss', state_loss, report_cnt)
                    tb_writer.add_scalar(
                        'train_loc_loss', loc_loss, report_cnt)

                    model.eval()
                    eval_score = evaluate(
                        dev_set, model, tb_writer, report_cnt)
                    model.train()
                    ...
                    if eval_score > best_score:  # new best score
                        ...
                        if opt.save_mode == 'all':
                            save_model(
                                opt.ckpt_dir, f'best_checkpoint_{best_score:.3f}.pt', model, optimizer)
                        elif opt.save_mode in ['best', 'best-last']:
                            save_model(
                                opt.ckpt_dir, f'best_checkpoint.pt', model, optimizer)
                    else:
                        impatience += 1
                        output(
                            f'Impatience: {impatience}, best score: {best_score:.3f}.')
                        if opt.save_mode == 'all':
                            save_model(
                                opt.ckpt_dir, f'checkpoint{report_cnt}_{eval_score:.3f}.pt', model, optimizer)
...
def evaluate(dev_set, model, tb_writer, report_cnt: int):
    dev_batch = DataLoader(
        dataset=dev_set, batch_size=opt.batch_size, shuffle=False, collate_fn=Collate())
    ...
    total_accuracy = (report_state_correct + report_loc_correct) / \
        (report_state_pred + report_loc_pred)
    state_accuracy = report_state_correct / report_state_pred
    loc_accuracy = report_loc_correct / report_loc_pred
...
def test(test_set, model):
    ...
    test_batch = DataLoader(
        dataset=test_set, batch_size=opt.batch_size, shuffle=False, collate_fn=Collate())
    ...
    with torch.no_grad():
        for batch in test_batch:
            ...
            for i in range(batch_size):
                pred_instance = get_output(
                    metadata=metadata[i], pred_state_seq=pred_state_seq[i], pred_loc_seq=pred_loc_seq[i])
    ...
    write_output(output=output_result,
                 dummy_filepath=opt.dummy_test, output_filepath=opt.output)
...
if __name__ == "__main__":
    ...
    if opt.mode == 'train':
        ...
        if not opt.output.endswith('.tsv'):
            print(
                "[WARNING] The output will be in TSV format, while the specified output file does not have .tsv suffix.")

        plm_model_class, plm_tokenizer_class, plm_config_class = MODEL_CLASSES[
            opt.plm_model_class]
        plm_tokenizer = plm_tokenizer_class.from_pretrained(opt.plm_model_name)

        test_set = ProparaDataset(
            opt.test_set, opt=opt, tokenizer=plm_tokenizer, is_test=True)
        ...
        model.eval()
        print(
            f'[INFO] Loaded model from {opt.restore}, time elapse: {time.time() - restore_start_time}s')
