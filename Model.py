...
class KOALA(nn.Module):
    ...
    def __init__(self, opt: argparse.Namespace, is_test: bool):
        ...
        plm_model_class, plm_tokenizer_class, plm_config_class = MODEL_CLASSES[
            opt.plm_model_class]

        self.plm_config = plm_config_class.from_pretrained(opt.plm_model_name)
        self.plm_tokenizer = plm_tokenizer_class.from_pretrained(
            opt.plm_model_name)

        # ConceptNet encoder
        if is_test:
            self.cpnet_encoder = plm_model_class(
                config=self.plm_config)  # use saved parameters
            print(
                f'[INFO] Loaded an empty {opt.plm_model_name} for ConceptNet encoder during testing')
        elif opt.cpnet_plm_path:
            self.cpnet_encoder = plm_model_class.from_pretrained(
                opt.cpnet_plm_path)
        ...
        if is_test:
            self.embed_encoder = plm_model_class(
                config=self.plm_config)  # use saved parameters
            print(
                f'[INFO] Loaded an empty {opt.plm_model_name} for embedding language model during testing')
        elif opt.wiki_plm_path:
            assert not opt.no_wiki, "Specified -no_wiki option but used a pre-fine-tuned BERT"
            self.embed_encoder = plm_model_class.from_pretrained(
                opt.wiki_plm_path)
            print(
                f'[INFO] Loaded {opt.wiki_plm_path} for embedding language model')
        else:
            self.embed_encoder = plm_model_class.from_pretrained(
                opt.plm_model_name)
            print(
                f'[INFO] Loaded {opt.plm_model_name} for embedding language model')
        ...
        self.CrossEntropy = nn.CrossEntropyLoss(
            ignore_index=PAD_LOC, reduction='mean')
    ...
    def forward(self, token_ids: torch.Tensor, entity_mask: torch.IntTensor,
        ...
        attention_mask = (
            token_ids != self.plm_tokenizer.pad_token_id).to(torch.int)
        plm_outputs = self.embed_encoder(
            token_ids, attention_mask=attention_mask)
        # hidden states at the last layer, (batch, max_tokens, plm_hidden_size)
        embeddings = plm_outputs[0]

        # (batch, max_tokens, 2*hidden_size)
        ...
        cpnet_rep = self.CpnetEncoder(
            cpnet_triples, tokenizer=self.plm_tokenizer, encoder=self.cpnet_encoder)
        ...
        log_likelihood = self.CRFLayer(emissions=tag_logits, tags=gold_state_seq.long(
        ), mask=tag_mask, reduction='token_mean')

        state_loss = -log_likelihood  # State classification loss is negative log likelihood
        pred_state_seq = self.CRFLayer.decode(
            emissions=tag_logits, mask=tag_mask)
        ...
        loc_logits, loc_attn_probs = self.LocationPredictor(encoder_out=token_rep, entity_mask=entity_mask,
                                                            ...
                                                            cpnet_triples=cpnet_triples, cpnet_rep=cpnet_rep)
        # size (batch, max_sents + 1, max_cands)
        loc_logits = loc_logits.transpose(-1, -2)
        masked_loc_logits = self.mask_loc_logits(
            loc_logits=loc_logits, num_cands=num_cands)  # (batch, max_sents + 1, max_cands)
        masked_gold_loc_seq = self.mask_undefined_loc(
            gold_loc_seq=gold_loc_seq, mask_value=PAD_LOC)  # (batch, max_sents + 1)
        ...
        if loc_attn_probs is not None:
            loc_attn_probs = self.get_gold_attn_probs(
                loc_attn_probs, gold_loc_seq)
        ...
        if self.is_test:  # inference
            pred_loc_seq = get_pred_loc(
                loc_logits=masked_loc_logits, gold_loc_seq=gold_loc_seq)
    ...
    def get_attn_loss(self, state_attn_probs, loc_attn_probs, state_rel_labels, loc_rel_labels):
        ...
        if state_attn_probs is not None:
            state_pos_attn_probs = state_attn_probs.masked_select(
                mask=state_rel_labels.to(torch.bool))  # 1-D tensor
        if loc_attn_probs is not None:
            loc_pos_attn_probs = loc_attn_probs.masked_select(
                mask=loc_rel_labels.to(torch.bool))  # 1-D tensor

        if state_attn_probs is not None and loc_attn_probs is not None:
            pos_attn_probs = torch.cat(
                [state_pos_attn_probs, loc_pos_attn_probs], dim=0)
        ...
        if pos_attn_probs is not None:
            ...
            attn_loss = self.BinaryCrossEntropy(
                input=pos_attn_probs, target=gold_labels)
    ...
    def get_gold_attn_probs(self, loc_attn_probs, gold_loc_seq):
        ...
        pick_loc_seq = gold_loc_seq.masked_fill(
            mask=(gold_loc_seq < 0), value=0)
        gold_attn_probs = []

        for i in range(batch_size):
            for j in range(max_sents):
                gold_attn_probs.append(
                    loc_attn_probs[i][pick_loc_seq[i][j]][j])
    ...
    def mask_loc_logits(self, loc_logits, num_cands: torch.IntTensor):
        ...
        range_tensor = range_tensor.unsqueeze(
            dim=0).expand(batch_size, max_cands)
        bool_range = torch.gt(range_tensor, num_cands.unsqueeze(
            dim=-1))  # find the off-limit positions
        assert bool_range.size() == (batch_size, max_cands)

        # use this bool tensor to mask loc_logits
        bool_range = bool_range.unsqueeze(dim=-2).expand_as(loc_logits)
    ...
    def mask_undefined_loc(self, gold_loc_seq, mask_value: int):
        ...
        masked_gold_loc_seq = gold_loc_seq.masked_fill(
            mask=negative_labels, value=mask_value)
...
class StateTracker(nn.Module):
    ...
    def __init__(self, opt: argparse.Namespace):
        ...
        self.Decoder = nn.LSTM(input_size=4 * opt.hidden_size, hidden_size=opt.hidden_size,
                               num_layers=1, batch_first=True, bidirectional=True)
        self.Dropout = nn.Dropout(p=opt.dropout)
        self.Hidden2Tag = Linear(
            d_in=2 * opt.hidden_size, d_out=NUM_STATES, dropout=0)
        self.CpnetMemory = CpnetMemory(
            opt, query_size=4 * opt.hidden_size, input_size=4 * opt.hidden_size)
    ...
    def forward(self, encoder_out, entity_mask, verb_mask, sentence_mask, cpnet_triples, cpnet_rep):
        ...
        decoder_in = self.get_masked_input(
            encoder_out, entity_mask, verb_mask, batch_size=batch_size)
        # (batch, max_sents, 4 * hidden_size)
        attn_probs = None
        ...
        # (batch, max_sents, 2 * hidden_size), forward & backward concatenated
        decoder_out, _ = self.Decoder(decoder_in)
        decoder_out = self.Dropout(decoder_out)
        # (batch, max_sents, num_tags)
        tag_logits = self.Hidden2Tag(decoder_out)
    ...
    def get_masked_input(self, encoder_out, entity_mask, verb_mask, batch_size: int):
        ...
        entity_rep = self.get_masked_mean(
            source=encoder_out, mask=entity_mask, batch_size=batch_size)
        # (batch, max_sents, 2 * hidden_size)
        verb_rep = self.get_masked_mean(
            source=encoder_out, mask=verb_mask, batch_size=batch_size)
        # (batch, max_sents, 4 * hidden_size)
        concat_rep = torch.cat([entity_rep, verb_rep], dim=-1)

        assert concat_rep.size() == (batch_size, max_sents, 4 * self.hidden_size)
        entity_existence = find_allzero_rows(
            vector=entity_mask).unsqueeze(dim=-1)  # (batch, max_sents, 1)
    ...
    def get_masked_mean(self, source, mask, batch_size: int):
        ...
        # turn binary masks to boolean values
        bool_mask = (mask.unsqueeze(dim=-1) == 0)
        masked_source = source.unsqueeze(dim=1).masked_fill(bool_mask, value=0)
        # sum the unmasked vectors
        masked_source = torch.sum(masked_source, dim=-2)
        assert masked_source.size() == (batch_size, max_sents, 2 * self.hidden_size)

        # compute the denominator of average op
        num_unmasked_tokens = torch.sum(mask, dim=-1, keepdim=True)
        # average the unmasked vectors
        masked_mean = torch.div(input=masked_source, other=num_unmasked_tokens)
...
class LocationPredictor(nn.Module):
    ...
    def __init__(self, opt: argparse.Namespace):
        ...
        self.Decoder = nn.LSTM(input_size=4 * opt.hidden_size, hidden_size=opt.hidden_size,
                               num_layers=1, batch_first=True, bidirectional=True)
        self.Dropout = nn.Dropout(p=opt.dropout)
        self.Hidden2Score = Linear(
            d_in=2 * opt.hidden_size, d_out=1, dropout=0)
        self.CpnetMemory = CpnetMemory(
            opt, query_size=4 * opt.hidden_size, input_size=4 * opt.hidden_size)
        self.cpnet_inject = opt.cpnet_inject
        unk_vec = torch.empty(2 * opt.hidden_size)
        nn.init.uniform_(unk_vec, -math.sqrt(1 / opt.hidden_size),
                         math.sqrt(1 / opt.hidden_size))
        # learnable vector for '?' location
    ...
    def forward(self, encoder_out, entity_mask, loc_mask, sentence_mask, cpnet_triples, cpnet_rep):
        ...
        decoder_in = self.get_masked_input(
            encoder_out, entity_mask, loc_mask, batch_size=batch_size)
        decoder_in = decoder_in.view(
            batch_size * max_cands, max_sents, 4 * self.hidden_size)
        attn_probs = None
        if self.cpnet_inject in ['location', 'both']:
            decoder_in, attn_probs = self.CpnetMemory(encoder_out=KOALA.expand_dim_3d(encoder_out, max_cands),
                                                      ...
                                                      entity_mask=KOALA.expand_dim_3d(
                                                          entity_mask, max_cands),
                                                      sentence_mask=KOALA.expand_dim_3d(
                                                          sentence_mask, max_cands),
                                                      cpnet_triples=cpnet_triples,
                                                      cpnet_rep=cpnet_rep,
                                                      loc_mask=loc_mask)
        # (batch, max_sents, 2 * hidden_size), forward & backward concatenated
        decoder_out, _ = self.Decoder(decoder_in)
        assert decoder_out.size() == (batch_size * max_cands,
                                      max_sents, 2 * self.hidden_size)

        decoder_out = decoder_out.view(
            batch_size, max_cands, max_sents, 2 * self.hidden_size)
        decoder_out = self.Dropout(decoder_out)
        loc_logits = self.Hidden2Score(decoder_out).squeeze(
            dim=-1)  # (batch, max_cands, max_sents)
    ...
    def get_masked_input(self, encoder_out, entity_mask, loc_mask, batch_size: int):
        ...
        loc_rep = self.get_masked_loc_mean(
            source=encoder_out, mask=loc_mask, batch_size=batch_size)
        unk_vec = self.unk_vec.expand(
            batch_size, max_sents, 2 * self.hidden_size)
        entity_existence = find_allzero_rows(
            vector=entity_mask).unsqueeze(dim=-1)  # (batch, max_sents, 1)
        unk_vec = unk_vec.masked_fill(mask=entity_existence, value=0).unsqueeze(
            dim=1)  # (batch, 1, max_sents, 2*hidden)
        ...
        assert entity_rep.size() == loc_rep.size() == (
            batch_size, max_cands + 1, max_sents, 2 * self.hidden_size)

        concat_rep = torch.cat([entity_rep, loc_rep], dim=-1)
        assert concat_rep.size() == (batch_size, max_cands + 1,
                                     max_sents, 4 * self.hidden_size)
    ...
    def get_masked_loc_mean(self, source, mask, batch_size: int):
        ...
        # turn binary masks to boolean values
        bool_mask = (mask.unsqueeze(dim=-1) == 0)
        # expand source to (batch, 1, 1, tokens, 2*hidden)
        source = source.unsqueeze(dim=1).unsqueeze(dim=1)
        # (batch, cands, sents, tokens, 2*hidden)
        masked_source = source.masked_fill(bool_mask, value=0)
        # (batch, cands, sents, 2*hidden)
        masked_source = torch.sum(masked_source, dim=-2)
        assert masked_source.size() == (batch_size, max_cands,
                                        max_sents, 2 * self.hidden_size)

        num_unmasked_tokens = torch.sum(
            mask, dim=-1, keepdim=True)  # (batch, cands, sents, 1)
        # (batch, cands, sents, 2*hidden)
        masked_mean = torch.div(input=masked_source, other=num_unmasked_tokens)
    ...
    def get_masked_mean(self, source, mask, batch_size: int):
        ...
        # turn binary masks to boolean values
        bool_mask = (mask.unsqueeze(dim=-1) == 0)
        masked_source = source.unsqueeze(dim=1).masked_fill(
            bool_mask, value=0)  # for masked tokens, turn its value to 0
        # sum the unmasked token representations
        masked_source = torch.sum(masked_source, dim=-2)
        assert masked_source.size() == (batch_size, max_sents, 2 * self.hidden_size)

        # compute the denominator of average op (number of unmasked tokens)
        num_unmasked_tokens = torch.sum(mask, dim=-1, keepdim=True)
        # average the unmasked vectors
        masked_mean = torch.div(input=masked_source, other=num_unmasked_tokens)
...
class CpnetMemory(nn.Module):
    ...
    def forward(self, encoder_out, decoder_in, entity_mask, sentence_mask, cpnet_triples: List[List[str]],
        ...
        if loc_mask is not None:
            assert cpnet_rep.size(
                0) != batch_size, batch_size % cpnet_rep.size(0) == 0
...
class GatedAttnUpdate(nn.Module):
    ...
    def __init__(self, query_size: int, value_size: int, input_size: int, dropout: float):
        ...
        self.gate_fc = Linear(input_size + value_size,
                              input_size, dropout=dropout)
        self.concat_fc = Linear(input_size + value_size,
                                input_size, dropout=dropout)
    ...
    def forward(self, query, values, ori_input, attn_mask, ori_batch_size: int):
        ...
        :return:
        """
        assert query.size(0) == values.size(
            0), query.size(1) == ori_input.size(1)
        assert len(query.size()) == len(
            values.size()) == len(ori_input.size()) == 3
        ...
        # attention
        attn_vec = self.attn_vec.unsqueeze(0).expand(
            batch_size, -1, -1)  # (batch, query_size, value_size)
        ...
        # attention weights, (batch, max_sents, num_cpnet)
        probs = F.softmax(S, dim=-1)
        if ori_batch_size != batch_size:
            self.attn_log.extend(probs.view(
                ori_batch_size, -1, max_sents, num_cpnet).tolist())
        else:
            self.attn_log.extend(probs.tolist())
        is_nan = torch.isnan(probs)
        # if no valid triple exist, the system will output nan
        probs = probs.masked_fill(is_nan, value=0)
        # weighted sum, (batch, max_sents, value_size)
        C = torch.bmm(probs, values).squeeze(dim=-1)
        ...
        final_input = torch.mul(gate_vec, cand_input) + \
            torch.mul(1 - gate_vec, ori_input)
...
class FixedSentEncoder(nn.Module):
    ...
    def forward(self, input: List[List[str]], tokenizer, encoder):
        ...
        input_ids = list(map(lambda s: tokenizer.convert_tokens_to_ids(
            s.split()), all_sents))  # should already add special tokens
        ...
        for batch_input_ids in input_batches:
            ...
            for i in range(last_hidden.size(0)):
                ...
                # get rid of <CLS> and <SEP>
                token_embed = embedding[real_tokens]
        ...
        sent_embed = sent_embed.view(
            batch_size, num_cpnet, 2 * self.hidden_size)
    ...
    def pad_to_longest(batch: List, pad_id: int) -> (torch.LongTensor, torch.FloatTensor):
        ...
        pad_batch = [
            batch[i] + [pad_id for _ in range(max_length - len(batch[i]))] for i in range(batch_size)]
        pad_batch = torch.tensor(pad_batch, dtype=torch.long)
        # avoid computing attention on padding tokens
        attention_mask = torch.ones_like(pad_batch).masked_fill(
            mask=(pad_batch == pad_id), value=0)
...
class Bilinear(nn.Module):
    ...
    def __init__(self, in_1: int, in_2: int, d_out: int, dropout: float, bias: bool = True):
        ...
        self.bilinear = nn.Bilinear(
            in1_features=in_1, in2_features=in_2, out_features=d_out, bias=bias)
        self.dropout = nn.Dropout(p=dropout)
